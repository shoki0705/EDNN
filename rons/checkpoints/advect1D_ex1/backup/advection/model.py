...
class Advection1DModel(BaseModel):
    ...
    def __init__(self, cfg):
        ...
        # 入力1次元、出力1次元のネットワークを作成
        self.field = self._create_network(1, 1) # 現在のベクトル場 
        self.field_prev = self._create_network(1, 1)    # ひとつ前のベクトル場
        self._set_require_grads(self.field_prev, False) # ひとつ前のベクトル場は勾配計算しない


    # 訓練可能なネットワーク（今回はself.field）を辞書形式で返す
    @property
    def _trainable_networks(self):
        return {"field": self.field}
    
    
    # 指定された解像度の1次元サンプルをself.length/2でスケーリング
